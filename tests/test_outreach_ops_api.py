...
class OutreachOpsApiTests(unittest.TestCase):
    ...
    def test_outreach_ops_lists_new_thread_backlog_for_latest_auto_jobs_only(self) -> None:
        ...
        self.assertIsNone(items[0].get("assigned_account_id"))
        self.assertIsNone(items[0].get("assigned_account_label"))
        self.assertNotIn("likely_account_id", items[0])
        self.assertNotIn("likely_account_label", items[0])
